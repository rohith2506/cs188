...
class ReflexAgent(Agent):
		...
		def get_ghost_score(self, ghostStates, pacmanPos):
			score = 0
			for ghost in ghostStates:
				...
				if ghostScaredTime <= 0:
					score -= max(4 - distanceToGhost, 0)
				else:
					score += max(5 - distanceToGhost, 0)
			return score
		...
		def evaluationFunction(self, currentGameState, action):
				...
				food_distance = map(lambda x: 1.0 / manhattanDistance(x, newPos), newFood.asList())
				food_score = max(food_distance + [0])
				ghost_score = self.get_ghost_score(newGhostStates, newPos)
				return food_score + ghost_score + successorGameState.getScore()
